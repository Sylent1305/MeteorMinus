...
public class AutoTrash extends Module
{
    ...
    private final Setting<Boolean> massDump = sgGeneral.add(new BoolSetting.Builder()
        .name("mass-dump")
        .description("Dumps all items into the trash bin in a single tick.")
        .defaultValue(false)
        .build()
    );
    ...
    private final Setting<Integer> delayDump = sgGeneral.add(new IntSetting.Builder()
        .name("dump-delay")
        .description("Ticks between moving items.")
        .defaultValue(2)
        .min(0)
    ...
    private void onTick(TickEvent.Pre event)
    {
        ...
        if (mc.currentScreen instanceof HandledScreen<?>)
        {
            if (massDump.get())
            {
                while (dumpOneItem((HandledScreen<?>) mc.currentScreen))
                {
                }
            ...
            else
            {
                ...
                if (dumpOneItem((HandledScreen<?>) mc.currentScreen))
                {
